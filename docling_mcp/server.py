...
def is_document_in_local_cache(cache_key: str) -> bool:
    ...
    Returns:
        ...
        success: bool
    """
    return key in local_document_cache
...
def convert_pdf_document_into_json_docling_document_from_uri_path(
    ...
    try:
        ...
        logger.info(f"Set up pipeline options")
        ...
        logger.info(f"Start conversion")
...
@mcp.tool()
def convert_attachment_into_docling_document(
    pdf_payload: Annotated[bytes, {"media_type": "application/octet-stream"}]
) -> Dict[str, Any]:
    """
    Process a pdf file attachment from Claude Desktop.
    
    Args:
        pdf_payload: PDF document as binary data from the attachment
